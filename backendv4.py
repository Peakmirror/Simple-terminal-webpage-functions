...
#def check_login():
    conn = sqlite3.connect("users.db")
    cursor = conn.cursor()
    check = cursor.fetchon

# Manage account function

def manage_account():
    action = input("What do you want to do? [c]hange password, [d]elete account or [s]et token(incase you forget your password): ")
...
def generate_token():
    return secrets.token_hex(32)  # Generates a secure random token

# Set token for user

def set_token_for_user(username, conn):
    token = generate_token()
    one_year_seconds = 365 * 24 * 60 * 60
    expiry = int(time.time()) + one_year_seconds
...
#Validatint the token
def is_token_valid(username, token, conn):
    cursor = conn.cursor()
    cursor.execute("""
